...
APP_TITLE = "Khánh Mail Checker V46 Avatar Splash"
APP_VERSION = "46"
DEFAULT_UPDATE_URL = ""
...
def check_emailscan(provider, emails, progress_cb=None):
    ...
    obj = api_post_json(
        ...
        headers={
            "Content-Type": "application/json",
            "x-api-key": api_key,
            "User-Agent": "Khanh-Mail-Checker/46.0"
        },
        timeout=300
    )
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(title_box, text="Avatar Splash • Mail • Thẻ", style="Sub.TLabel").pack(anchor="w")
        ...
        self.status_var = tk.StringVar(value="Sẵn sàng. V46 có avatar app và màn hình chào.")
    ...
    def setup_app_avatar(self):
        try:
            avatar = resource_path("app_avatar.png")
            if avatar.exists():
                self._app_icon_img = tk.PhotoImage(file=str(avatar))
    ...
    def show_start_splash(self):
        try:
            ...
            avatar = resource_path("app_avatar.png")
            img = tk.PhotoImage(file=str(avatar))
            # Thu ảnh cho vừa splash nếu ảnh lớn
            try:
                w, h = img.width(), img.height()
                factor = max(1, int(max(w / 220, h / 220)))
                if factor > 1:
            ...
            splash._img = img
            box = tk.Frame(splash, bg="white", padx=22, pady=18)
            box.pack(fill="both", expand=True)

            tk.Label(box, image=img, bg="white").pack()
            tk.Label(
                box,
                text="Khánh Mail Checker",
                bg="white",
                fg="#0f172a",
    ...
    def open_update_tool(self):
        ...
        def parse_manifest_or_zip(url):
            u = str(url or "").strip()
            if not u:
                raise RuntimeError("Bạn chưa nhập link update.")

            clean_url = u.lower().split("?")[0].split("#")[0]
            if clean_url.endswith(".zip"):
                return {"version": "", "zip_url": u, "notes": "Direct zip"}

            manifest_file = download_file(u, suffix=".json")
            raw_bytes = manifest_file.read_bytes()

            # Nếu link có query nhưng tải về là zip thật.
            if raw_bytes[:2] == b"PK":
                return {"version": "", "zip_url": u, "notes": "Direct zip"}
            ...
            if not stripped:
                raise RuntimeError(
                    "Link update trả về dữ liệu trống.\n"
                    "Hãy dùng link raw/direct của manifest.json hoặc link tải trực tiếp .zip."
                )

            low = stripped[:300].lower()
            if low.startswith("<!doctype html") or low.startswith("<html") or "<html" in low:
                raise RuntimeError(
                    "Link bạn dán đang trả về trang web HTML, không phải manifest.json.\n\n"
                    "Nếu dùng GitHub: bấm Raw rồi copy link raw.githubusercontent.com.\n"
                    "Nếu dùng file zip: dùng link tải trực tiếp .zip."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    "manifest.json không đúng định dạng JSON.\n\n"
                    f"Lỗi: {e}\n\n"
                    f"Nội dung nhận được: {preview}"
                )
